...
open class ObjectEmailtypeApi(basePath: kotlin.String = defaultBasePath, client: Call.Factory = ApiClient.defaultClient) : ApiClient(basePath, client) {
    companion object {
        @JvmStatic
    ...
    /**
     * To obtain the request config of the operation emailtypeGetAutocompleteV2
     *
     * @param sSelector The type of Emailtypes to return
     * @param eFilterActive Specify which results we want to display. (optional, default to EFilterActive.Active)
     * @param sQuery Allow to filter the returned results (optional)
    ...
    fun emailtypeGetAutocompleteV2RequestConfig(sSelector: SSelectorEmailtypeGetAutocompleteV2, eFilterActive: EFilterActiveEmailtypeGetAutocompleteV2?, sQuery: kotlin.String?, acceptLanguage: HeaderAcceptLanguage?) : RequestConfig<Unit> {
        ...
        return RequestConfig(
            ...
            path = "/2/object/emailtype/getAutocomplete/{sSelector}".replace("{"+"sSelector"+"}", encodeURIComponent(sSelector.value.toString())),
