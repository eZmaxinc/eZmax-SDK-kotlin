
# CustomEzmaxpartnerListElement

## Properties
| Name | Type | Description | Notes |
| ------------ | ------------- | ------------- | ------------- |
| **pkiEzmaxpartnerID** | **kotlin.Int** | The unique ID of the Ezmaxpartner |  |
| **sEzmaxpartnerAddressX** | **kotlin.String** | The complete address in a single line |  |
| **sEzmaxpartnerEmailaddressX** | **kotlin.String** | The email address. |  |
| **sEzmaxpartnerShortdescriptionX** | **kotlin.String** | The short description of the Ezmaxpartner in the language of the requester |  |
| **sEzmaxpartnerNameX** | **kotlin.String** | The name of the Ezmaxpartner in the language of the requester |  |
| **sEzmaxpartnerPhoneE164X** | **kotlin.String** | A phone number in E.164 Format |  |
| **sEzmaxpartnerUrlX** | **kotlin.String** | The url of the Ezmaxpartner website in the language of the requester |  |



